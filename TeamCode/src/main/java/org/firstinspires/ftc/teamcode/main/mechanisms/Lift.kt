...
object Lift : Subsystem {
    ...
    var SPEED = 1.0
    @JvmField
    var HIGH_POSITION = 22.0 // in
    @JvmField
    var MEDIUM_POSITION = 15.5 // in
    @JvmField
    var LOW_POSITION = 11.0 // in
    @JvmField
    var DIRECTION = DcMotorSimple.Direction.FORWARD

    // unconfigurable constants
    private const val PULLEY_WIDTH = 2.0 // in
    private const val COUNTS_PER_REV = 537.6 // NeveRest 20 orbital (really 19.2 ratio, not 20)
    ...
    val toMiddle: Command
        get() = MotorToPosition(liftMotor, (MEDIUM_POSITION * COUNTS_PER_INCH).toInt(), SPEED, listOf(this))
    val toHigh: Command
        get() = MotorToPosition(liftMotor, (HIGH_POSITION * COUNTS_PER_INCH).toInt(), SPEED, listOf(this))
    val toStart: Command
        get() = MotorToPosition(liftMotor, (0.1 * COUNTS_PER_INCH).toInt(), SPEED, listOf(this))
    val raise: Command
